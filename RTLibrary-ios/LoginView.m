...
- (void)createView{
    ...
    [[self.loginButton rac_signalForControlEvents:UIControlEventTouchUpInside]
     subscribeNext:^(id x) {
         ...
         if(self.loginClick){
             self.loginClick(self.userNameField.text,self.passwordField.text,self);
         }
     }];
    ...
    RAC(self.loginButton,enabled) = [RACSignal
                                     combineLatest:@[
                                                     ...
                                                     RACObserve(self, isConnected)
                                                     ] reduce:^(NSString *username, NSString *password,NSNumber* connent) {
                                                         // NSLog(@"%@",password);
                                                         return @(username.length > 0 && password.length > 0 && [connent boolValue]);
                                                     }];
